...
| --- | --- |
| `SamedisCare.Api` | Auth, HTTP, gridfilter, tenant scope, record lookup and the cascades |
| `SamedisCare.Helper` | Logging, dates, CSV, text encodings, config |
...
tasks with documents — generated by `make_fixtures.py` and shared with the other two syncs so
an end-to-end run can chain them.
...
- The run goes **uploads first, then downloads**, and within each in dependency order:
  device types, device models, departments, locations, inventories, tasks, requests. An
  inventory cannot be written before the room and the device model it points at exist, and a
  task cannot be written before its inventory does.
...
Device model download note (`devicemodels.csv`):
- `application_risk` carries `risk_level` unchanged (`unknown` / `0` / `1` / `2`) — the
  Anwendungsrisiko, i.e. how much instruction a user needs.
- `training_mandatory` is derived from it: `Ja` for 1 and 2, `Nein` for 0, empty for
  `unknown`. It used to be derived from `operator_ordinance`, which answers a different
  question — the annex says which rules a device falls under, the risk level says whether
  someone has to be shown how to use it.
- `risk_class` carries the MDR risk class from `regulatory.eu_mdr` as `I` / `IIa` / `IIb` /
